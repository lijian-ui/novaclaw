...
| **核心思路** | LLM 驱动�?Agent 委托（delegate_task�?| 注册表驱动的任务/Flow 生命周期管理 | SessionTask trait + �?Agent 线程�?|
| **任务分解方式** | `delegate_task` 工具（同进程�?Agent�?| `sessions_spawn` 工具（子会话/ACP�?| `spawn_agent` 控制平面（独立线程） |
| **深度控制** | 最�?3 层（配置可调�?| 支持嵌套（orchestrator/leaf 角色�?| Agent �?`AgentPath` 深度不限 |
| **并行方式** | ThreadPoolExecutor | 独立 gateway 会话 | 独立 OS 线程 |
| **持久�?* | 看板 SQLite（跨 profile�?| TaskRegistry SQLite + FlowRegistry | AgentGraphStore SQLite（边持久化） |
| **中断机制** | 全局�?Agent 注册�?+ 中断传播 | subagents(steer/kill) | `send_input`/`close_agent`/级联取消 |
| **任务追踪** | `todo` 工具 + 看板 | Task + Flow 双注册表 | SessionTask + TurnContext |
| **通知/结果** | �?Agent 自动返回 | Task 投递策略（done/state/silent�?| 事件�?+ Rollout 持久�?|
| **技能系�?* | SKILL.md 文档驱动（writing-plans, subagent-driven-dev�?| TaskFlow 技能包 | core-skills 渲染�?|
| **工具规划** | ToolRegistry 动�?schema | `buildToolPlan()` 可用性评�?| ToolDef 框架 |
...
�?             Tools（工具层 - LLM 可调用）                      �?
�? todo    �?当前会话内的任务列表管理                             �?
�? delegate_task �?派生�?Agent（单任务 / 批量并行�?             �?
�? kanban_* �?跨会�?�?Profile 的看板任务调�?                   �?
└──────────────────────┬──────────────────────────────────────�?
                       �?
┌──────────────────────┴──────────────────────────────────────�?
...
| 文件 | 大小 | 用�?|
|------|------|------|
| `tools/delegate_tool.py` | 2768 �?| **核心** �?Agent 委托/�?Agent 架构 |
| `tools/todo_tool.py` | 278 �?| 任务列表管理工具 |
| `tools/registry.py` | ~600 �?| 中心工具注册表（动�?schema�?|
| `toolsets.py` | �?| 工具集定义（delegation/todo/kanban�?|
| `hermes_cli/kanban_db.py` | 4000+ �?| **核心** �?看板数据�?& 调度�?|
| `agent/prompt_builder.py` | �?| 系统提示构建（含编排指导�?|
| `skills/software-development/writing-plans/SKILL.md` | �?| 编写实现计划的技�?|
| `skills/software-development/subagent-driven-development/SKILL.md` | �?| �?Agent 驱动开发技�?|
| `skills/devops/kanban-orchestrator/SKILL.md` | �?| 看板编排器技�?|
...
DELEGATE_TASK_SCHEMA = {
    ...
    "parameters": {
        "goal": "str (optional)",           # 单任务模�?
        "tasks": "[Task] (optional)",        # 批量并行模式
        "role": "'leaf' | 'orchestrator'",   # 子角�?
        "toolsets": "[str]",                 # 子工具集
        "context": "str",                    # 上下�?
        "max_concurrency": "int",            # 并发�?
    }
}
```

- **阻止的工�?*：`delegate_task`, `clarify`, `memory`, `send_message`, `execute_code`（防递归死循环）
- **默认深度**�?（仅 parent→child），最大可配至 3
...
```yaml
delegation:
  max_concurrent_children: 3       # 并行�?Agent �?
  max_iterations: 50               # �?Agent 迭代预算
  max_spawn_depth: 1               # 深度 [1-3]
  orchestrator_enabled: true       # 编排器开�?
  child_timeout_seconds: 600       # 超时
```

### 2.6 关键设计要点

1. **�?Plan Mode 开�?*：规划通过 `todo` �?`delegate_task` 的工具描述驱动，而非全局模式切换
2. **�?Agent 彻底隔离**：每个子 Agent 是完�?`AIAgent` 实例，独立上下文、终端、API 预算
3. **Kanban �?Profile 共享**：所�?profile 共享同一�?SQLite 看板数据�?
4. **动�?Schema 覆盖**：`delegate_task` 的描述在每次调用时根据运行时配置动态生成，LLM 看到的是实际限制
...
| 文件 | 大小 | 用�?|
|------|------|------|
| `src/tasks/task-registry.ts` | 2173 �?| **核心** �?任务注册�?|
| `src/tasks/task-executor.ts` | 697 �?| 任务执行�?|
| `src/tasks/task-flow-registry.ts` | 706 �?| Flow 注册�?|
| `src/agents/tools/sessions-spawn-tool.ts` | 465 �?| **核心** �?子任务生成工�?|
| `src/agents/tools/subagents-tool.ts` | 185 �?| �?Agent 管理(list/kill/steer) |
| `src/agents/subagent-spawn.ts` | �?| �?Agent 生成逻辑 |
| `src/agents/subagent-system-prompt.ts` | �?| �?Agent 系统提示 |
| `src/agents/system-prompt.ts` | �?| 主系统提示（含编排指导） |
| `src/tools/planner.ts` | �?| 工具规划器（可用性评估） |
| `skills/taskflow/SKILL.md` | �?| TaskFlow 技能使用指�?|
...
type TaskRecord = {
  taskId: string;                    // 唯一 ID
  runtime: "subagent" | "acp" | "cli" | "cron";  // 运行时类�?
  requesterSessionKey: string;       // 请求方会�?
  ownerKey: string;                  // 所有�?
  childSessionKey?: string;          // 子会话（子代�?ACP�?
  parentFlowId?: string;             // 所�?Flow
  parentTaskId?: string;             // 父任务（重试链）
  agentId?: string;                  // 目标 Agent
  task: string;                      // 任务描述
  status: "queued" | "running" | "succeeded" | "failed" 
        | "timed_out" | "cancelled" | "lost";  // 7 种状�?
...
type TaskFlowRecord = {
  ...
  status: "queued" | "running" | "waiting" | "blocked" 
        | "succeeded" | "failed" | "cancelled" | "lost";  // 8 种状�?
  goal: string;                      // 目标
  currentStep?: string;              // 当前步骤
  blockedTaskId?: string;            // 阻塞的任�?ID
  revision: number;                  // 乐观锁版本号
  stateJson?: JsonValue;             // 持久化状�?
  waitJson?: JsonValue;              // 等待元数�?
};
```
...
�?Agent 会话
  ...
  �?  └─ callGateway �?启动�?Agent 会话
  �?
  ├─ �?Agent 运行 �?自动投递结�?
  �?
  └─ Task 完成 �?TaskRegistry 更新状�?
```
...
5. finish({ stateJson }) �?fail({ ... })
6. requestCancel / cancel
```

所有有状态的修改都使�?`expectedRevision` 进行乐观锁冲突检测�?
...
1. **双注册表设计**：TaskRegistry（单体任务） + TaskFlowRegistry（流程编排），职责分�?
2. **ACP 协议支持**：`sessions_spawn` 支持 `acp` 运行时，跨机�?Agent 通信
3. **上下文模�?*：子 Agent 可选择 `isolated`（隔离）�?`fork`（继承父上下文）
4. **乐观�?*：Flow 状态修改使�?revision 版本号防冲突
...
�? Agent �? Thread Tree �?父子关系 �?Edge 持久�?             �?
�? 级联关闭: 父关�?�?所有在线后代关�?                         �?
└──────────────────────┬──────────────────────────────────────�?
                       �?
┌──────────────────────┴──────────────────────────────────────�?
�?              Agent Registry（Agent 注册表）                   �?
�? Agent �? HashMap<ThreadId, AgentMetadata>                  �?
...
�? 边状�? active / completed / failed / orphaned              �?
�? 用于恢复和监�?                                             �?
└─────────────────────────────────────────────────────────────�?
```
...
| `codex-rs/core/src/tasks/review.rs` | ReviewTask �?独立审查子会�?|
| `codex-rs/core/src/tasks/user_shell.rs` | UserShellCommandTask �?Shell 命令 |
| `codex-rs/core/src/agent/control.rs` | **核心** �?Agent 控制平面 |
| `codex-rs/core/src/agent/registry.rs` | Agent 注册�?|
| `codex-rs/agent-graph-store/` | Agent 图谱 SQLite 持久�?|
| `codex-rs/collaboration-mode-templates/` | 协作模式提示词模�?|
...
用户输入
  ...
  �?  ├─ LLM 生成 �?检查系统提示中�?agent 工具
  �?  �?
  �?  ├─ LLM 调用 spawn_agent 工具
  �?  �?  ├─ 解析 SpawnRequest（role/model/system_prompt�?
  �?  �?  ├─ AgentControl::spawn_agent_with_metadata()
  �?  �?  �?  ├─ 创建�?OS 线程
  �?  �?  �?  ├─ 注册�?AgentRegistry（设�?agent_path�?
  �?  �?  �?  ├─ 持久化边�?AgentGraphStore
  �?  �?  �?  └─ 启动�?Session + SessionTask
  �?  �?  �?
  �?  �?  ├─ �?Agent 独立运行
  �?  �?  �?  ├─ 可进一�?spawn �?Agent（递归�?
  �?  �?  �?  └─ 完成后通过 Rollout 持久化结�?
  �?  �?  �?
  �?  �?  └─ �?Agent 通过 send_input 与子 Agent 通信
  �?  �?
  �?  ├─ LLM 调用 send_inter_agent_communication
  �?  �?  └─ �?Agent 收到消息并处�?
  �?  �?
  �?  ├─ 关闭�?Agent �?close_agent(agent_id, reason)
  �?  �?  └─ 级联关闭所有后�?
  �?  �?
  �?  └─ 会话结束 �?所�?Agent 关闭
```
...
`collaboration-mode-templates/` 目录包含�?Agent 协作的系统提示词模板�?

```rust
pub enum CollaborationMode {
    /// 一个核�?worker + 多个独立 explorer，并行探索后汇�?
    Research,  
    /// 一�?orchestrator 管理多个 worker，分发并汇�?
    Orchestrated,
...
2. **OS 线程级隔�?*：每个子 Agent 在独立线程运行，彻底隔离
3. **Agent �?+ 级联关闭**：父子关系持久化�?SQLite，关闭时级联取消所有后�?
4. **Rollout 持久�?*：子 Agent 结果通过 Rollout 机制持久化，支持断点恢复
5. **协作模式**：三种预定义�?Agent 协作模式（Research/Orchestrated/AssemblyLine�?
6. **ReviewTask 独立会话**：审查作为独�?SessionTask 运行，不阻塞主流�?
...
```
┌──────────────────�?  ┌──────────────────�?  ┌──────────────────�?
�? Agent 委托�?    �?  �? 注册表驱动式     �?  �? 线程树驱动式    �?
�? (hermes-agent)  �?  �? (openclaw)      �?  �? (codex)         �?
�?                 �?  �?                 �?  �?                 �?
�? LLM 自主调用     �?  �? 强类型注册表     �?  �? Rust trait 抽象  �?
�? delegate_task   �?  �? TaskRegistry    �?  �? SessionTask     �?
�? 同进程子 Agent   �?  �? SQLite 持久�?   �?  �? OS 线程隔离      �?
�? 线程池并�?      �?  �? 乐观锁防冲突     �?  �? Agent 树级�?   �?
�? Kanban 跨会�?  �?  �? ACP 跨机�?     �?  �? Rollout 持久�? �?
└──────────────────�?  └──────────────────�?  └──────────────────�?
```
...
|------|-------------|----------|-------|
| **任务分解执行�?* | LLM 通过工具调用分解 | LLM 通过 sessions_spawn 工具分解 | Agent 通过控制平面 API 分解 |
| **子任务隔�?* | 同进�?AIAgent 实例 | 独立 gateway 会话 | 独立 OS 线程 |
| **深度控制** | 最�?3 层（配置�?| 支持嵌套（orchestrator/leaf�?| Agent 树深度不�?|
| **持久�?* | 看板 SQLite | Task + Flow �?SQLite | 图谱�?SQLite |
| **中断机制** | 全局注册�?+ 中断传播 | subagents(steer/kill) | send_input / 级联 close |
| **结果传�?* | �?Agent 自动返回 | Task 投�?+ 通知策略 | Rollout 事件�?|
| **并发控制** | ThreadPoolExecutor | 独立 Gateway 会话 | OS 线程 + 注册表限�?|
| **审查机制** | 两阶段（Implementer→Reviewer�?| 内建 ReviewTask | ReviewTask 独立会话 |
| **工具规划** | 动�?schema 覆盖 | buildToolPlan 可用性评�?| ToolDef 框架 |
| **学习/进化** | �?| �?| �?|

### 5.3 �?jeeves 的启�?

1. **子任务工具是刚需**：三个项目都通过 spawn/subagent/delegate 工具�?LLM 自主分解任务。jeeves 缺少这类工具�?

2. **隔离 vs 共享**�?
   - codex 提供最强隔离（OS 线程），但开销�?
   - hermes-agent 同进程隔离，轻量但风险高
   - openclaw 独立会话，平衡了隔离和开销

3. **持久化是关键**：openclaw �?Task + Flow 双注册表设计最适合 jeeves 的场景——任务状态持久化�?SQLite，支持恢复和监控�?

4. **中断机制**：三个项目都实现了不同程度的中断（steer/kill/close），jeeves 目前缺少这个能力�?

5. **深度控制**：hermes-agent �?openclaw 都有深度限制（最�?3 层），codex 不限深度但有级联关闭。jeeves 应当加深度限制防递归失控�?

6. **工具规划**：openclaw �?`buildToolPlan()` —�?动态过滤不可用工具并告�?—�?对限制子 Agent 能力范围很有价值�?
