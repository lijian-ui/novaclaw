...
> **日期**: 2026-05-09
> **状态**: 设计草案（待实现）
> **基于**: Anthropic Skills 规范、Hermes Agent 实现、NovaClaw 当前 Skills 基础设施
...
5. [Phase 3: 技能体验优化](#5-phase-3-技能体验优化)
...
- ✅ `SkillsLoader` — SKILL.md 文件解析、扫描、安装、删除
- ✅ `skill_view` 工具 — LLM 按名称加载技能详情（`name` + `description` 双字段元数据）
- ✅ `skills_list` 工具 — LLM 列出所有可用技能
...
%LOCALAPPDATA%/novaclaw/skills/  # 技能存储目录
...
| Phase | 功能 | 依赖 | 优先级 | 预估工作量 |
|-------|------|------|--------|-----------|
| **1** | Skills Hub 集成 | 无 | P1 | 3-5 天 |
| **2** | 安全扫描系统 | Phase 1 | P1 | 2-3 天 |
| **3** | 技能体验优化 | Phase 1 | P2 | 2-4 天 |
| **4** | 多作用域支持 | 无 | P3 | 1-2 天 |
...
允许用户从 [agentskills.io](https://agentskills.io) 或 Anthropic Skills 官方仓库安装社区技能，并为发布自定义技能提供标准接口。
...
---                            # 仅需要 name + description 两个字段

# Skill Title
Full instructions in Markdown...
```

**Hermes Agent 的 Skills Hub 集成方式：**

```
CLI 命令：
  /skills           → 列出所有可用技能
  /<skill-name>     → 直接调用技能
  hermes claw migrate → 从 OpenClaw 导入技能

外部来源：
  ~/.hermes/skills/ → 用户本地技能
...
pub struct SkillDef {
    ...
    // === 新增字段（Phase 1）===
    /// 技能来源: "local" | "hub" | "plugin"
    pub source: SkillSource,

    /// 技能出处 URL（如果是 Hub 安装的）
    pub source_url: Option<String>,

    /// 分类/标签（用于 Skills Hub 浏览）
...
完整 SKILL.md 的 YAML frontmatter 解析扩展：
...
```rust
// src-tauri/src/cmd.rs 或 backend 路由
...
→ 调用 install_skill({name: "pdf-analysis", source: "hub"})
  → 从 agentskills.io 下载 SKILL.md
  → 安装到本地
  → 返回安装成功
→ LLM 再调用 skill_view 加载详情
→ 执行技能
```

#### 3.3.3 Tauri 命令层扩展

如果 NovaClaw 有前端 UI 管理技能，通过 Tauri 命令桥接：
...
防止恶意或不安全的 SKILL.md 被安装到系统中，保护用户免受提示注入攻击。

### 4.2 参考实现

**Hermes Agent 的注入检测模式**（基于 `_INJECTION_PATTERNS`）：
...
#### 4.3.1 注入检测（在 `install_skill` / `parse_skill_md` 时执行）
...
当前 `skill_view` 工具只通过 `SkillsLoader::get_skill(name)` 按名称查找，不涉及路径参数，**没有路径遍历攻击面**。但如果将来 `skill_view` 支持 `file_path` 参数（如 Hermes 那样读取技能目录内的子文件），就需要防护：
...
impl SkillPreprocessor {
    ...
    pub fn preprocess(content: &str, context: &PreprocessContext) -> String {
        ...
        // 1. 展开环境变量 ${HOME}, ${USER} 等
        result = Self::expand_env_vars(&result);

        // 2. 替换模板变量 {{workspace_dir}}, {{os_name}} 等
...
根据 Anthropic Skills 规范，前端元数据仅需 `name` + `description`。Hermes Agent 增加了更多可选字段。NovaClaw 可以支持以下扩展：
  ...
  novaclaw:
    config:
      - key: review.style
        ...
        default: ~/.novaclaw/review-style.md
---
...
支持不同层级的技能作用域，解决技能命名冲突，实现按项目/用户/系统分层的技能管理。
...
| 作用域 | 优先级 | 路径 | 说明 |
|--------|--------|------|------|
| **Repo** | 0（最高） | 仓库内 `.agents/skills/` | 项目特有技能 |
| **User** | 1 | `$HOME/.agents/skills/` | 用户个人技能 |
| **System** | 2 | `$CODEX_HOME/skills/.system/` | 内置系统技能 |
| **Admin** | 3（最低） | `/etc/codex/skills/` | 管理员部署 |

去重策略：按 `(作用域优先级, 技能名称)` 排序，同名的保留高优先级。
...
pub enum SkillScope {
    /// 项目级技能（仓库内 .novaclaw/skills/）
    Project,
    /// 用户级技能（%LOCALAPPDATA%/novaclaw/skills/）
    User,
    /// 系统级技能（内置，只读）
    System,
}
...
%LOCALAPPDATA%/novaclaw/skills/.system/
├── skill-creator/SKILL.md
└── summarizer/SKILL.md

# 用户安装（Phase 1 已有）
%LOCALAPPDATA%/novaclaw/skills/pdf_analysis/SKILL.md

# 项目级（在仓库根目录）
<project_root>/.novaclaw/skills/
...
| 链接 | 用途 |
|------|------|
| [anthropics/skills](https://github.com/anthropics/skills) | 官方仓库，示例技能 |
| [agentskills.io](https://agentskills.io) | Skills 开放标准规范 |

### 7.2 关键文件路径（NovaClaw 当前）
...
| `backend/src/server/routes/skills.rs` | 技能 HTTP API |
| `docs/agent工具调用优化方案.md` | 工具传递优化讨论记录 |
| `docs/skills/hermes-agent-skills-analysis.md` | Hermes Agent 技能系统分析 |
| `docs/skills/codex-skills-analysis.md` | Codex 技能系统分析 |
| `docs/skills/claw-code-skills-analysis.md` | Claw-Code 技能系统分析 |
...
理由：
- Phase 1 是功能入口，先打通 Hub 才有第三方技能来源
- Phase 2 是 Phase 1 的安全保障，必须先做
- Phase 3 和 Phase 4 是体验优化，不阻塞核心功能
```
