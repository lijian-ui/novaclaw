    ...
    std::thread::spawn(move || {
        rt.block_on(async move {
                ...
                handler: std::sync::Arc::new(move |args: serde_json::Value, _chunk_tx: Option<tokio::sync::mpsc::UnboundedSender<String>>| -> Result<String, String> {
                    ...
                    let result = std::thread::spawn(move || {
                        ...
                        rt.block_on(async move {
                            ...
                            let no_fallback = tinyfish_api_key.as_ref().map_or(true, |k| k.is_empty())
                            ...
                            if results.is_empty() {
                                let msg = if last_error.is_empty() {
                                    ...
                                    format!("{}\n提示：DuckDuckGo 可能被网络限制，请在设置中配置 TinyFish 或 Tavily 搜索 API Key 作为备用搜索引擎", last_error)
                                } else {
            ...
            registry_clone.register(ToolDef {
                ...
                handler: std::sync::Arc::new(|args: serde_json::Value, chunk_tx: Option<tokio::sync::mpsc::UnboundedSender<String>>| -> Result<String, String> {
                    let agent_id = args["agent_id"].as_str().ok_or("Missing 'agent_id' parameter")?.to_string();
                    let task = args["task"].as_str().ok_or("Missing 'task' parameter")?.to_string();
                    ...
                    tracing::info!("[SubAgent] 委托任务给 '{}' ({}): {}", agent_config.id, agent_config.name, task);
                    ...
                    let result = rt.block_on(async {
                        ...
                        let (provider, config, full_registry) = {
                            let provider = state.models_config.find_provider_by_model(&model_to_use)
                                .ok_or_else(|| format!("未找到模型 '{}' 的提供商配置", model_to_use))?
                                .clone();
                        ...
                        let mut sub_session = crate::agent::session::AgentSession::new(
                            &agent_config.name, &model_to_use, None,
                        );
                        ...
                        match agent.run_turn("", None, None, &[]).await {
                            Ok(result) => {
                                tracing::info!("[SubAgent] '{}' 任务完成，输出 {} 字符", agent_config.name, result.content.len());
                                if let Some(ref tx) = chunk_tx {
                                    let _ = tx.send(serde_json::json!({
                                ...
                                Ok(format!(
                                    ...
                                    agent_config.name, result.content, agent_config.name,
                                    result.iterations, result.total_input_tokens, result.total_output_tokens,
                                ))
                            }
                            Err(e) => {
